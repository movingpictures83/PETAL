...
def read_params(params):
    ...
    gl.pathway_input = params["pathway"]
    print(f'Pathway: {gl.pathway_input}')

    gl.gene_input = params["gene"]
    print(f'Gene: {gl.gene_input}')

    gl.deep_input = int(params["depth"])
    print(f'Depth: {gl.deep_input}')

    gl.num_cores_input = int(params["cpus"])
...
def export_data_for_deep(deep, path):
    gl.DF_TREE.to_csv(os.path.join(path, f'df_resulted_deep_{deep}.csv'), sep=';',
                      header=False, index=False)

    gl.DF_TREE.to_csv(os.path.join(path, 'df_resulted.csv'), sep=';', mode='a',
                      header=False, index=False)
